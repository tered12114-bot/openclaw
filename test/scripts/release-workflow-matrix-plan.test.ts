...
describe("scripts/plan-release-workflow-matrix.mjs", () => {
  it("keeps beta release jobs to beta-enabled Docker E2E chunks and live model providers", () => {
    ...
    expect(plan.liveModels.matrix.include.map((entry) => entry.providers)).toEqual(["openai"]);
  ...
  it("requires new release-profile matrices to use a planner or an explicit allowlist", () => {
    expect(staticProfileMatrixJobs()).toEqual(PROFILE_GATED_STATIC_MATRIX_ALLOWLIST);
  });
});
